...
class SingleProduct extends AbstractTemplate implements TemplateInterface
{
    protected $requiresMatrix = true;

    public function execute()
    {
        $this->logger->debug("Creating a single product");
        $queue = $this->queue;
        ...
        if ($token = $this->shop->google_access_token) {
        ...
        switch($this->shop->myshopify_domain) {
            ...
            case 'importer-testing.myshopify.com':
                $this->matrix = json_decode(file_get_contents(DIR.'/src/wholesale.json'), true);
                if (!$this->matrix) {
        ...
        $sku = $this->generateSku($this->shop, $queue->title);
        $results['product_name'] = $queue->title;
        $results['front_print_file_url'] = $queue->front_print_url;
        $results['back_print_file_url'] = $queue->back_print_url;
        ...
        foreach($images as $garment => $img) {
            ...
            foreach ($img as $color => $src) {
                ...
                $variantSku = $this->getVariantSku($sku, $garment, $color);
                ...
                $variantSettings = $this->matrix[$garment];
                foreach($variantSettings['sizes'] as $size => $sizeSettings) {
                    ...
                    $varData = array(
                        ...
                        'option1' => $this->getSku($size),
                        'option2' => $color,
                        'option3' => (($this->shop->myshopify_domain == 'forged-blue.myshopify.com' && $garment == 'Tank') ? 'Women\'s Tank' : $garment),
        ...
        $results['shopify_product_admin_url'] = "https://{$this->shop->myshopify_domain}/admin/products/{$res->product->id}";
        $variantMap = array();
        $imageUpdate = array();
        ...
        $this->logger->debug(json_encode($imageUpdate));
        ...
        if (isset($queue->log_to_google) && $queue->log_to_google && $this->shop->google_access_token) {
            logResults($this->client, $this->shop->google_sheet_slug, $post['print_type'], $results, $this->shop->id);
        } else {
